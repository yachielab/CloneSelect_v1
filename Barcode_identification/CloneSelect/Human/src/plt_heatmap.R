...
df.all <- read.csv("data/GFP_isolation/barcode_counts_rep2_normalized_long.csv", header = TRUE, sep = ",")
df.AUG <- read.csv("data/GFP_isolation/barcode_counts_rep2_atg_normalized_long.csv", header = TRUE, sep = ",")

# df.rank <- read.csv("./barcode_rank_plot_07182022.csv", header = TRUE, sep = ",")
df.rank <- read_excel("merged_96lib_pDNA_gDNA_01082023.xlsx")

# merge for all data (GTG and AUG)
df.all.merged <- merge(df.all, df.rank, by.x = "bc", by.y = "clone_name") %>%
    arrange(gDNA_raw) %>%
    mutate(new_order = 1:length(order))

# merge for AUG data
df.AUG.merged <- merge(df.AUG, df.rank, by.x = "bc", by.y = "clone_name") %>%
    arrange(gDNA_raw) %>%
    mutate(new_order = 1:length(order))

# plot heatmap for all barcode enrichment
g.heatmap.all <- ggplot(df.all.merged, aes(y = reorder(query_bc, -order), x = reorder(bc, order))) +
...
g.heatmap.AUG <- ggplot(df.AUG.merged, aes(y = reorder(query_bc, -order), x = reorder(bc, order))) +
    geom_tile(aes(fill = norm_bc), colour = NA) +
    theme_classic() +
    theme(
        panel.background = element_blank(),
