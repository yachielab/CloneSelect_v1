...
df <- read.csv("data/GFP_isolation/rep1_result/barcode_counts_rep1.tsv", header = TRUE, sep = "\t")
...
ggsave(plot = g.heatmap.all, filename = "BC_heatmap_01112023.pdf", device = cairo_pdf, w = 5.2, h = 2.6)
...
g.jitter <- ggplot(df.merged, aes(x = reorder(clone_name, -new_order), y = norm_bc_freq)) +
    ...
    labs(x = "", y = "") +
    guides(colour = "none")
ggsave(plot = g.jitter, filename = "BC_jitter_01112022_.pdf", device = cairo_pdf, w = 2.21, h = 1.2)