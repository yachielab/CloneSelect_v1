...
df.mean.CBE$col_str <- factor(df.mean.CBE$col_str, c("C-to-TBC2", "C-to-TBC4", "C-to-TBC6", "C-to-TBC8", "C-to-TBC10", "C-to-TBC12"))

df.mean.gRNA <- df.mean %>%
    dplyr::filter(System == "gRNA")
df.mean.gRNA$col_str <- factor(df.mean.gRNA$col_str, c("gRNABC2", "gRNABC4", "gRNABC6", "gRNABC8", "gRNABC10", "gRNABC12"))

df.mean.CRISPRa <- df.mean %>%
    dplyr::filter(System == "CRISPRa")
df.mean.CRISPRa$col_str <- factor(df.mean.CRISPRa$col_str, c("CRISPRaBC2", "CRISPRaBC4", "CRISPRaBC6", "CRISPRaBC8", "CRISPRaBC10", "CRISPRaBC12"))
